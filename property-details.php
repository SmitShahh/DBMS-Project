          ...
          <!-- <h3 class="h4 text-black widget-title mb-3 ml-0">ADD THIS TO FAVORITE</h3> -->
              ...
              <?php if(isset($_SESSION['id'])) :?>
                <form action="favs/add-fav.php" class="form-contact-agent" method="POST">
                  ...
                  <div class="form-group">
                    <input type="hidden" id="email" name="user_id" value="<?php echo $_SESSION['id']; ?>" class="form-control">
                  </div>
                  <?php if($check->rowCount()>0) : ?>
                    <div class="form-group">
                      <a href="favs/delete-fav.php?prop_id=<?php echo $id; ?>&user_id=<?php echo $_SESSION['id']; ?>" class="btn btn-primary" text-white>Delete From Favorite</a>
                    </div>
                  <?php else : ?>
                    <div class="form-group">
                    <input type="submit" name="submit" id="submit" class="btn btn-primary" value="Add to favorite">
                    </div>
                  <?php endif; ?>
                </form>
              <?php else: ?>
